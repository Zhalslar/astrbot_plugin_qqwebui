...
export function sessionAvatarUrl(session) {
  if (!session) {
    return "";
  }
  if (text(session.chat_type).trim() === "private") {
    return avatarUrl(session.target_id, "private");
  }
  return text(session.avatar).trim() || avatarUrl(session.target_id, session.chat_type);
}
